  ...
  return (
    <div className={className}>
      ...
      <div className="space-y-3 md:hidden">
        {rows.map((row, i) => (
          <div key={i} className="rounded-lg border border-gray-100 dark:border-slate-700 bg-white dark:bg-slate-800 p-3 shadow-sm">
            {columns.map((col) => (
              <div key={col.key} className="flex items-center justify-between py-1">
                ...
                <span className="text-sm font-semibold text-gray-700 dark:text-gray-200">{row[col.key]}</span>
              </div>
            ))}
          </div>
        ))}
