...
async function apiFetch<T>(endpoint: string, params: Record<string, string>): Promise<T[]> {
  ...
  if (json.errors && Object.keys(json.errors).length > 0) {
    console.warn(`[api-football] API errors for ${endpoint}:`, json.errors);
    // Don't call forceExhaust — per-minute rate limits are transient and the
    // serial queue (250ms gap) naturally prevents sustained bursts. Blocking
    // all calls for 5 min on a per-minute error causes more harm than good.
    return [];
  }
