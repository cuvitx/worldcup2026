...
export const metadata: Metadata = {
  ...
  description:
    "Entdecken Sie die heutigen Spiele der WM 2026. Spielzeiten, Mannschaften, Stadien und Links zu den Prognosen jeder Begegnung.",
...
<section className="hero-animated text-white py-12 sm:py-16">
        <div className="relative z-10 mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
          <h1 className="text-2xl font-extrabold sm:text-4xl">Spiele heute</h1>
          <p className="mt-2 text-gray-300">
            {todaysMatches.length > 0
              ? `${todaysMatches.length} Spiel${todaysMatches.length > 1 ? "e" : ""} heute`
              : "Keine Spiele heute geplant"}
          </p>
        </div>
      </section>
      ...
      <div className="relative z-10 mx-auto max-w-7xl px-4 py-8">
        {todaysMatches.length > 0 ? (
              ...
              return (
                ...
                >
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <span className="text-lg" role="img" aria-label={`Flagge von ${home?.name ?? "Unbekannt"}`}>{home?.flag ?? "\u{1F3F3}\u{FE0F}"}</span>
                  ...
                  <div className="flex items-center gap-2 flex-1 min-w-0 justify-end">
                    ...
                    <span className="text-lg" role="img" aria-label={`Flagge von ${away?.name ?? "Unbekannt"}`}>{away?.flag ?? "\u{1F3F3}\u{FE0F}"}</span>
                  </div>
                  {match.group && (
          ...
          <div className="rounded-xl border border-gray-200 bg-white shadow-sm p-5 hover:shadow-md transition-shadow text-center">
            ...
            {nextMatch && (() => {
              ...
              return (
                <div className="mt-4">
                  <p className="text-sm text-gray-500 mb-3">Nachstes Spiel:</p>
                  <Link
                  ...
                  >
                    <p className="text-sm text-gray-500 mb-1">
                      {matchDate} um {nextMatch.time}
                    </p>
                    <p className="font-semibold text-gray-900">
                      <span role="img" aria-label={`Flagge von ${home?.name ?? "Unbekannt"}`}>{home?.flag}</span> {home?.name ?? "Noch offen"} vs{" "}
                      {away?.name ?? "Noch offen"} <span role="img" aria-label={`Flagge von ${away?.name ?? "Unbekannt"}`}>{away?.flag}</span>
                    </p>
                    {stadium && (
        ...
        <div className="mt-8 text-center">
          ...
          >
            Vollstandigen Spielplan anzeigen
          </Link>
        </div>
      </div>
    </>
