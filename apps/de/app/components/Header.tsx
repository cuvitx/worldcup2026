...
const SearchDialog = dynamic(
  ...
  {
    loading: () => (
      <div className="w-9 h-9 rounded-lg animate-pulse bg-white/10" aria-label="Suche wird geladen" />
    ),
    ssr: false,
  }
);
...
export function Header() {
  ...
  const searchData = useMemo(() => buildSearchIndex("de"), []);
    ...
    <header className="hero-animated sticky top-0 z-50 backdrop-blur-[12px] text-white border-b border-white/10">
      ...
      <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3.5">
        ...
        <div className="flex items-center gap-1.5">
          <SearchDialog lang="de" data={searchData} onNavigate={(href) => router.push(href)} />
