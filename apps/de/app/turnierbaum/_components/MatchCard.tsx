...
function TeamSlot({ teamId, isWinner, label }: { teamId: string | null; isWinner?: boolean; label?: string }) {
  ...
  return (
    ...
    >
      <span className="text-base" role="img" aria-label={`Flagge von ${team.name}`}>{team.flag}</span>
...
export function MatchCard({
  ...
  const dateStr = match
    ? new Date(match.date + "T" + match.time + ":00+02:00").toLocaleDateString("de-DE", { day: "numeric", month: "short" })
    : "";

  return (
    <div className={`rounded-lg border ${colors.border} ${colors.bg} overflow-hidden min-w-[220px]`}>
      {match ? (
        <Link href={`/spiel/${match.slug}`} className="block">
