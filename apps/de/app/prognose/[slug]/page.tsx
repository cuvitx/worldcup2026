...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    ...
    openGraph: {
      ...
      description: `Cotes et pronostics ${team.name} pour la WM 2026. Analyse complète, probabilités et prédictions.`,
    },
  };
}

export default async function PrognoseTeamPage({ params }: PageProps) {
  const { slug } = await params;
  const team = teamsBySlug[slug];
  if (!team) notFound();
