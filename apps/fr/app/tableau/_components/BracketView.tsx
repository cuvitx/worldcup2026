...
import type { BracketData } from "./bracket-data";

export function DesktopBracket({ data }: { data: BracketData }) {
  const { r32Bracket, r32Winners, r16Matches, r16Winners, qfMatches, qfWinners, sfMatches, sfWinners, finalMatchData, champion } = data;
    ...
    <section className="hidden lg:block overflow-x-auto">
      <div className="flex gap-6 items-start min-w-max pb-4">
        ...
        <div className="space-y-3 shrink-0">
          <div className="text-center mb-4"><span className={`inline-block px-3 py-1 rounded-full text-xs font-bold text-white ${roundColors["round-of-32"]!.badge}`}>16es de finale</span></div>
          {r32Bracket.map((m, i) => (
        ...
        <div className="space-y-3 shrink-0 pt-[36px]">
          <div className="text-center mb-4"><span className={`inline-block px-3 py-1 rounded-full text-xs font-bold text-white ${roundColors["round-of-16"]!.badge}`}>8es de finale</span></div>
          {r16Matches.map((m, i) => (
...
export function MobileBracket({ data }: { data: BracketData }) {
  const { r32Bracket, r32Winners, r16Matches, r16Winners, qfMatches, qfWinners, sfMatches, sfWinners, finalMatchData, champion } = data;

  return (
    <section className="lg:hidden overflow-x-auto -mx-4 px-4">
      <div className="flex gap-4 items-start min-w-max pb-4">
        <div className="space-y-2 shrink-0 w-[220px]">
          <div className="text-center mb-3"><span className={`inline-block px-2.5 py-1 rounded-full text-xs font-bold text-white ${roundColors["round-of-32"]!.badge}`}>16es</span></div>
          {r32Bracket.map((m, i) => (
            <MatchCard key={m.matchId} matchId={m.matchId} homeTeamId={m.homeTeamId} awayTeamId={m.awayTeamId} homeLabel={m.homeLabel} awayLabel={m.awayLabel} winnerId={r32Winners[i] ?? null} stage="round-of-32" />
          ))}
        </div>
        <div className="space-y-2 shrink-0 w-[220px] pt-[28px]">
          <div className="text-center mb-3"><span className={`inline-block px-2.5 py-1 rounded-full text-xs font-bold text-white ${roundColors["round-of-16"]!.badge}`}>8es</span></div>
          {r16Matches.map((m, i) => (
        ...
        <div className="space-y-2 shrink-0 w-[220px] pt-[80px]">
        ...
        <div className="space-y-2 shrink-0 w-[220px] pt-[180px]">
        ...
        <div className="space-y-2 shrink-0 w-[220px] pt-[350px]">
