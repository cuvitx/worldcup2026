...
import { Users, Star, ShieldAlert, UserX } from "lucide-react";
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-12">
        ...
        <div className="rounded-lg bg-secondary/10 border border-secondary/30 p-4 mb-8">
          <p className="text-sm text-gray-700 dark:text-gray-300 font-medium">
            📋 <strong>Liste provisoire</strong> — sera confirmée par le sélectionneur en mai 2026. Les joueurs listés ci-dessous sont les plus probables selon les dernières convocations.
          </p>
        </div>
        ...
        <div className="grid gap-8 lg:grid-cols-3">
          <div className="lg:col-span-2 space-y-10">
              ...
              <section className="rounded-xl bg-white dark:bg-slate-800 p-6 shadow-sm">
                ...
                <div className="grid gap-4 sm:grid-cols-3">
                  {stars.map((player) => (
                    ...
                    >
                      <p className="text-lg font-bold text-gray-900 dark:text-white">{player.name}</p>
            ...
            {sortedPositions.map((pos) => {
              ...
              return (
                <section key={pos} className="rounded-xl bg-white dark:bg-slate-800 p-6 shadow-sm">
                    ...
                    <table className="w-full text-left text-sm">
                      <thead>
                        <tr className="border-b border-gray-200 dark:border-slate-700">
                          <th className="py-3 pr-4 font-semibold text-gray-700 dark:text-gray-300">Poste</th>
                          <th className="py-3 pr-4 font-semibold text-gray-700 dark:text-gray-300">Nom</th>
                          <th className="py-3 pr-4 font-semibold text-gray-700 dark:text-gray-300">Club</th>
                          <th className="py-3 pr-4 font-semibold text-gray-700 dark:text-gray-300">Âge</th>
                          <th className="py-3 font-semibold text-gray-700 dark:text-gray-300">Sélections</th>
                        </tr>
                      </thead>
                      <tbody>
                        {posPlayers.map((player) => {
                          ...
                          return (
                            <tr
                              key={player.id}
                              className={`border-b border-gray-100 dark:border-slate-700/50 ${isStar ? "bg-secondary/5" : ""}`}
                            >
                              ...
                              <td className="py-3 pr-4">
                                <Link href={`/joueur/${player.slug}`} className="font-medium text-accent hover:underline">
                                  {player.name}
                                  {isStar && <span className="ml-1 text-secondary">⭐</span>}
                                </Link>
                              </td>
                              <td className="py-3 pr-4 text-gray-600 dark:text-gray-400">{player.club}</td>
                              <td className="py-3 pr-4 text-gray-600 dark:text-gray-400">{player.age} ans</td>
                              <td className="py-3 text-gray-600 dark:text-gray-400">{player.caps}</td>
                            </tr>
                          );
                        })}
                      </tbody>
            ...
            {allPlayers.length === 0 && (
              <section className="rounded-xl bg-white dark:bg-slate-800 p-6 shadow-sm text-center">
            ...
            <section className="rounded-xl bg-white dark:bg-slate-800 p-6 shadow-sm">
              ...
              <div className="mt-4 rounded-lg bg-gray-50 dark:bg-slate-700/30 p-4">
          ...
          <aside className="space-y-6">
            <div className="rounded-xl bg-white dark:bg-slate-800 p-6 shadow-sm">
              <h3 className="font-bold text-gray-900 dark:text-white mb-3">Résumé effectif</h3>
              <div className="space-y-2 text-sm">
                {sortedPositions.map((pos) => (
                  <div key={pos} className="flex justify-between">
                    <span className="text-gray-500">{positionLabels[pos] ?? pos}s</span>
                    <span className="font-bold text-gray-900 dark:text-white">{grouped[pos]!.length}</span>
                  </div>
                ))}
