...
function getSidebarLinks(pathname: string): SidebarLink[] {
  ...
  if (pathname.startsWith("/joueur/") && slug) {
    ...
    return [
      ...
      { href: `/comparateur-joueurs`, label: "Comparer les joueurs", icon: <BarChart3 className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
  ...
  if (pathname.startsWith("/match/") && slug && slug !== "calendrier") {
    return [
      ...
      { href: `/corners/${slug}`, label: "Consulter les cotes corners", icon: <Target className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
  ...
  if (pathname.startsWith("/equipe/") && slug) {
    ...
    return [
      ...
      { href: `/parier/${slug}`, label: `Consulter les cotes`, icon: <CircleDollarSign className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
  ...
  if (pathname.startsWith("/ville/") && slug) {
    ...
    return [
      ...
      { href: `/hebergement/${slug}`, label: `Hébergement ${name}`, icon: <Plane className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
  ...
  if (pathname.startsWith("/stade/") && slug) {
    return [
      ...
      { href: `/billets`, label: "Acheter des billets", icon: <CircleDollarSign className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
    ];
  }
  ...
  if (pathname.startsWith("/bookmaker/") && slug) {
    return [
      ...
      { href: `/guide-paris`, label: "Explorer le guide paris", icon: <Target className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
  ...
  return [
    ...
    { href: `/meilleurs-bookmakers`, label: "Comparer les bookmakers", icon: <CircleDollarSign className="h-4 w-4 text-[#022149] dark:text-gray-300" /> },
...
export function ContextualSidebar({ pathname: pathnameProp }: ContextualSidebarProps) {
  ...
  return (
    <aside className="hidden lg:block sticky top-24 space-y-2">
      <p className="text-xs font-semibold uppercase tracking-widest text-[#022149] dark:text-gray-400 mb-3 px-3">
        Sur le même sujet
      </p>
      {links.map((link) => (
        <Link
          key={link.href}
          href={link.href}
          className="flex items-center gap-3 rounded-lg border border-gray-200 dark:border-slate-700 p-3 text-sm text-gray-700 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-slate-800 transition-colors group"
        >
          {link.icon}
          <span className="flex-1">{link.label}</span>
          <ChevronRight className="h-4 w-4 text-gray-300 dark:text-gray-600 group-hover:text-[#00B865] transition-colors" />
        </Link>
      ))}
    </aside>
  );
