          ...
          <div className="flex flex-wrap gap-2 justify-center">
            {SUGGESTIONS.map((s) => (
              <button
                ...
                className="px-4 py-2 rounded-full text-sm font-medium bg-gray-100 dark:bg-gray-700 text-gray-700 dark:text-gray-200 hover:bg-primary/10 hover:text-primary dark:hover:text-accent transition-colors"
              >
                {s.label}
              </button>
            ))}
              ...
              return (
                <section key={type} aria-labelledby={`group-${type}`}>
                    ...
                    {items.slice(0, activeType === "all" ? 6 : 50).map((item) => (
                      ...
                      >
                        ...
                        <div className="min-w-0 flex-1">
                          <div className="font-medium text-gray-900 dark:text-gray-100 text-sm truncate group-hover:text-primary dark:group-hover:text-accent transition-colors">
                            {item.title}
                          </div>
                        ...
                        <svg className="w-4 h-4 text-gray-300 dark:text-gray-600 shrink-0 group-hover:text-primary dark:group-hover:text-accent transition-colors" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
                  ...
                  {activeType === "all" && items.length > 6 && (
                    <button
                      onClick={() => setActiveType(type)}
                      className="mt-2 text-sm text-primary dark:text-accent hover:underline font-medium"
                    >
