...
export default function ParisMiTempsPage() {
  ...
  const schemaItems = [
    { name: "Accueil", url: "/" },
    { name: "Paris mi-temps / fin de match", url: "/paris-mi-temps" },
  ];
      ...
      <div className="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8 py-10 sm:py-12 space-y-14">
            ...
            <table className="w-full text-sm text-left">
              <thead>
                <tr className="border-b border-gray-200 dark:border-slate-700">
                  <th className="py-3 px-4 font-semibold text-gray-900 dark:text-white">HT/FT</th>
                  <th className="py-3 px-4 font-semibold text-gray-900 dark:text-white">Description</th>
                  <th className="py-3 px-4 font-semibold text-gray-900 dark:text-white">Fréquence CDM</th>
                  <th className="py-3 px-4 font-semibold text-gray-900 dark:text-white">Cote type</th>
                </tr>
              </thead>
              <tbody>
                {resultatsHtFt.map((r) => (
                  <tr key={r.combo} className="border-b border-gray-100 dark:border-slate-800">
                    <td className="py-3 px-4 font-bold text-accent">{r.combo}</td>
                    <td className="py-3 px-4 text-gray-700 dark:text-gray-300">{r.description}</td>
                    <td className="py-3 px-4 text-gray-600 dark:text-gray-400">{r.frequence}</td>
                    <td className="py-3 px-4 font-semibold text-gray-900 dark:text-white">{r.coteType}</td>
                  </tr>
                ))}
              </tbody>
            </table>
        ...
        <section>
          ...
          <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {statsCDM.map((s) => (
              <div key={s.stat} className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5 text-center">
                <p className="text-2xl font-extrabold text-accent">{s.valeur}</p>
                <p className="font-semibold text-gray-900 dark:text-white text-sm mt-1">{s.stat}</p>
        ...
        <section>
          ...
          <div className="space-y-4">
            {strategies.map((s) => (
              <div key={s.titre} className="rounded-xl border border-gray-200 dark:border-slate-700 bg-white dark:bg-slate-800 p-5">
                <h3 className="font-bold text-gray-900 dark:text-white">{s.titre}</h3>
