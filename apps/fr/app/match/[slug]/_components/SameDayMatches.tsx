          ...
          {sameDayMatches.slice(0, 5).map((m) => {
            ...
            return (
              <Link
                key={m.slug}
                href={`/match/${m.slug}`}
                className="flex items-center gap-2 w-full rounded-xl border border-gray-200 px-3 py-2.5 transition-all hover:border-primary/30 hover:shadow-sm"
              >
                <span className="text-sm font-bold text-primary tabular-nums shrink-0">
                  {m.time}
                </span>
                <span className="text-base shrink-0" role="img" aria-label={mHome?.name ?? ""}>{mHome?.flag ?? ""}</span>
                <span className="text-xs font-semibold text-gray-900 truncate flex-1">{mHome?.name ?? "TBD"}</span>
                <span className="text-[10px] font-bold text-gray-400 shrink-0">vs</span>
                <span className="text-xs font-semibold text-gray-900 truncate flex-1 text-right">{mAway?.name ?? "TBD"}</span>
          ...
          {sameDayMatches.map((m) => {
            ...
            return (
              ...
              >
                ...
                <span className="text-sm font-semibold text-gray-900 truncate flex-1">{mHome?.name ?? "TBD"}</span>
                <span className="text-xs font-bold text-gray-400 shrink-0">vs</span>
                <span className="text-sm font-semibold text-gray-900 truncate flex-1 text-right">{mAway?.name ?? "TBD"}</span>
