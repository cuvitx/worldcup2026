      ...
      <ul className="space-y-1.5 mb-5">
        {starters.map((entry) => (
          <li key={entry.player.id} className="flex items-center gap-2">
            ...
            <span className="text-[10px] uppercase text-gray-400 ml-auto shrink-0">
  ...
  return (
    <div className="bg-white rounded-xl border border-gray-200 shadow-sm overflow-hidden">
      ...
      <div className="flex flex-col sm:flex-row sm:divide-x sm:divide-gray-200">
        <TeamColumn lineup={home} teamName={homeName} flag={homeFlag} />

        {/* Mobile divider */}
        <div className="border-t border-gray-200 sm:hidden" />

        <TeamColumn lineup={away} teamName={awayName} flag={awayFlag} />
      </div>
    </div>
  );
}
