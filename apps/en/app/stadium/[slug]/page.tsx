...
export async function generateMetadata({ params }: PageProps): Promise<Metadata> {
  ...
  return {
    alternates: getAlternates("stadium", slug, "en"),
    title: `${stadium.name} - World Cup 2026 Stadium | ${stadium.city}`,
    description: `Complete guide to ${stadium.name} in ${stadium.city} for the 2026 World Cup. Capacity ${stadium.capacity.toLocaleString("en-US")} seats. ${stadium.description}`,
  };
}
...
export default async function StadiumPage({ params }: PageProps) {
  ...
  return (
    <>
      <BreadcrumbSchema items={[{name:"Home",url:"/"},{name:"Stadiums",url:"/stadiums"},{name:stadium.name,url:`/stadium/${stadium.slug}`}]} baseUrl={domains.en} />
      {/* Breadcrumbs */}
      <nav className="bg-white border-b border-gray-200">
        <div className="mx-auto max-w-7xl px-4 py-3">
          <ol className="flex items-center gap-2 text-sm text-gray-500">
            ...
            <li>/</li>
            <li><Link href="/stadiums" className="hover:text-primary">Stadiums</Link></li>
            <li>/</li>
      ...
      <div className="mx-auto max-w-7xl px-4 py-8">
        <div className="grid gap-8 lg:grid-cols-3">
            ...
            {stadiumMatches.length > 0 && (
              <section className="rounded-lg bg-white p-6 shadow-sm">
                <h2 className="mb-4 text-xl font-bold">
                  Matches at {stadium.name} ({stadiumMatches.length})
                </h2>
          ...
          <div className="space-y-6">
              ...
              <dl className="space-y-3 text-sm">
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium">
                    {city ? (
                      <Link href={`/city/${city.slug}`} className="text-accent hover:underline">
                        {stadium.city}
                      </Link>
                    ) : (
                      stadium.city
                ...
                <div className="flex justify-between">
                  ...
                  <dd className="font-medium">{stadium.capacity.toLocaleString("en-US")}</dd>
                </div>
                <div className="flex justify-between">
            ...
            {/* Other stadiums */}
            <div className="rounded-lg bg-white p-6 shadow-sm">
              <h3 className="mb-4 text-lg font-bold">Other Stadiums</h3>
              <ul className="space-y-2 text-sm">
                {stadiums
                  ...
                  .map((s) => (
                    <li key={s.id}>
                      <Link href={`/stadium/${s.slug}`} className="hover:text-accent">
                        {s.name} ({s.city})
                      </Link>
                    </li>
                  ))}
